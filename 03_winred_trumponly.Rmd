...
#load data files with just trump/rnc donations to winred created in step 01
winred_contribs <- readRDS("processed_data/winred_itemcontribs_trump_rnc.rds")

#create before/after election flag
winred_contribs <- winred_contribs %>% 
  mutate(
    before_after_election = if_else(date < "2020-11-03", "BEFORE", "AFTER")
  )
...
winred_trump_before_after <- winred_contribs %>% 
  group_by(received_by, before_after_election) %>% 
  summarise(total_dollars = sum(amount)) %>% 
  pivot_wider(names_from = before_after_election, values_from = total_dollars)

winred_trump_before_after

winred_trump_before_after %>% write_csv("output/winred_trump_before_after.csv")
